...
function call() external payable {
    ...
    /// @notice Reveal dice one‐by‐one for first three, then two at once
    function rollDice() external {
        ...
        if (currentState == GameState.Player1Roll1 && idx == 0) {
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(1))
            ));
            playerDice[idx][0] = uint8(rnd % 6) + 1;
            ...
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(1), playerDice[0][0])
            ));
            playerDice[idx][0] = uint8(rnd % 6) + 1;
            emit DiceRolled(msg.sender, playerDice[idx]);
        ...
        // die-2
            ...
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(2))
            ));
            playerDice[idx][1] = uint8(rnd % 6) + 1;
            emit DiceRolled(msg.sender, playerDice[idx]);
            ...
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(2), playerDice[0][1])
            ));
            playerDice[idx][1] = uint8(rnd % 6) + 1;
            emit DiceRolled(msg.sender, playerDice[idx]);
        ...
        // die-3
            ...
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(3))
            ));
            playerDice[idx][2] = uint8(rnd % 6) + 1;
            emit DiceRolled(msg.sender, playerDice[idx]);
            ...
            uint256 rnd = uint256(keccak256(
                abi.encodePacked(block.timestamp, msg.sender, uint8(3), playerDice[0][2])
            ));
            playerDice[idx][2] = uint8(rnd % 6) + 1;
            emit DiceRolled(msg.sender, playerDice[idx]);
        ...
        // final two at once
            ...
            for (uint8 i = 3; i < 5; i++) {
                uint256 rnd = uint256(keccak256(
                    abi.encodePacked(block.timestamp, msg.sender, i, playerDice[0][i])
                ));
                playerDice[idx][i] = uint8(rnd % 6) + 1;
            }
            emit DiceRolled(msg.sender, playerDice[idx]);
