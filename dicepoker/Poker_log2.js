// Interactive CLI for the DicePoker contract (4 betting rounds, sequential reveals) with full logging
...
// new contract
const CONTRACT_ADDRESS = "0x71DDcedF0166f6fAC0ec49CB8b8675b752DDB69E";
...
async function main() {
  ...
  while (true) {
    ...
    if ((state === 5 || state === 6) && meIdx !== -1) {
      ...
      if (meIdx === turn) menu.push({ desc: "Reveal die 1", fn: rollDice });
    }
    // Reveal die 2
    if ((state === 11 || state === 12) && meIdx !== -1) {
      const turn = (state === 11 ? 0 : 1);
      if (meIdx === turn) menu.push({ desc: "Reveal die 2", fn: rollDice });
    }
    // Reveal die 3
    if ((state === 17 || state === 18) && meIdx !== -1) {
      const turn = (state === 17 ? 0 : 1);
      if (meIdx === turn) menu.push({ desc: "Reveal die 3", fn: rollDice });
    }
    // Reveal final two dice
    if ((state === 23 || state === 24) && meIdx !== -1) {
      const turn = (state === 23 ? 0 : 1);
      if (meIdx === turn) menu.push({ desc: "Reveal final 2 dice", fn: rollDice });
    }
    // Always allow showing hands
