...
async function main() {
  ...
  const nft      = new ethers.Contract(
    "0xbac75f3b581933277740f697228a0b754505997a", // 0x8ea8ce47f0c48ce699df9826675f136fef4e8800
    [
  ...
  console.log("🔍 provider.getBlockNumber():", currentBlock);

  // 2) get the receipt for tx1
  const receipt1 = await provider.getTransactionReceipt(tx1.hash);
  console.log("🔍 provider.getTransactionReceipt(tx1):", receipt1);

  // 3) get current network gas price
  const feeData = await provider.getFeeData();
  const networkGasPrice = feeData.gasPrice;
  console.log(
    "🔍 provider.getFeeData().gasPrice:",
