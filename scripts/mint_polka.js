...
// ── Tweakable gas settings ───────────────────────────────────────────────────
// Change these to experiment with different gas prices/limits:
const GAS_PRICE_GWEI = "50";    // e.g. "10", "20", "50", "100"
...
/** sample tx/s over the last `sampleSize` blocks */
...
async function main() {
  ...
  const nft = new ethers.Contract(
    "0xda1f2f9737ebdfc694605b9ce050964bd03bd6e1",
    [
  ...
  console.log(`   ▶ using gasPrice=${GAS_PRICE_GWEI} gwei, gasLimit=${GAS_LIMIT}`);
  const tx1 = await nft.safeMintWithMetrics(wallet.address, overrides1);
